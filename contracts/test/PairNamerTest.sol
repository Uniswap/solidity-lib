...
contract PairNamerTest {
    function pairName(address token0, address token1, string calldata prefix, string calldata suffix) external view returns (string memory) {
        return PairNamer.pairName(token0, token1, prefix, suffix);
    }

    function pairSymbol(address token0, address token1, string calldata suffix) external view returns (string memory) {
